...
final class BenefitCardCell: UITableViewCell {
    ...
    func configureCard(_ item: BenefitCard) {
        ...
        case .available:
            ...
            card.backgroundColor = .fdSurface
            card.alpha = 1
            showActions(gift: !item.transferredOnce, primary: "立即兑换")
        case .redeemed:
            let day = item.redeemedAt.map { String($0.prefix(10)) } ?? "--"
    ...
    func configureTransfer(_ item: BenefitTransferRecord, countdown: String) {
        ...
        if item.status == .waiting {
            metaLabel.font = .fdCaptionSemibold
            metaLabel.text = countdown
        } else {
            metaLabel.font = .fdCaption
            let name = (item.recipientName ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
            let claimed = Self.formatClaimed(item.claimedAt)
    ...
    private func setupUI() {
        ...
        let coverIcon = UIImageView(image: UIImage(systemName: "gift.fill"))
        coverIcon.tintColor = .fdPrimary
        coverIcon.contentMode = .scaleAspectFit
        let coverTitle = UILabel()
        coverTitle.text = "权益卡"
        coverTitle.font = .fdMicroSemibold
        coverTitle.textColor = .fdPrimary
    ...
    private static func formatClaimed(_ value: String?) -> String {
