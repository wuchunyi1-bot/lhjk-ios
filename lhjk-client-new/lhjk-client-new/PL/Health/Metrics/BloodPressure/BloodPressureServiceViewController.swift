...
/// 血压服务首页 — 对齐源项目 `ADBloodPressureServiceController`
final class BloodPressureServiceViewController: BaseViewController {

    private enum Row: Int, CaseIterable {
        case head, entry, advice
    }

    private let viewModel = BloodPressureServiceViewModel()
    ...
    override func setupUI() {
        ...
        tableView.register(BloodPressureServiceHeadCell.self, forCellReuseIdentifier: BloodPressureServiceHeadCell.reuseID)
    ...
    override func bindViewModel() {
        ...
        viewModel.$adviceTitle
            ...
            .receive(on: DispatchQueue.main)
            .sink { [weak self] _ in self?.tableView.reloadRows(at: [IndexPath(row: Row.advice.rawValue, section: 0)], with: .none) }
            .store(in: &cancellables)
...
extension BloodPressureServiceViewController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int { Row.allCases.count }

    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        switch Row(rawValue: indexPath.row) {
        case .head:
            let cell = tableView.dequeueReusableCell(withIdentifier: BloodPressureServiceHeadCell.reuseID, for: indexPath) as! BloodPressureServiceHeadCell
        ...
        case .advice:
            ...
            cell.onMoreTap = { [weak self] in
                Router.shared.push("/health/metrics/blood-pressure/detail", params: [:])
            }
            return cell
        case .none:
