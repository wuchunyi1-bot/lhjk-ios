...
地址编辑交互：所在地区由用户手动选择省市区，保存接口字段不变。不提供定位回填。
...
#### Scenario: 字段结构
- **THEN** 页面分区标题为「收货信息」「默认设置」
- **AND** 字段为：收货人、手机号、所在地区（展示行 + 右侧箭头）、详细地址、设为默认地址、保存地址
- **AND** 占位文案：收货人「请输入收货人姓名」；手机号「请输入收货人手机号码」；所在地区「请选择省、市、区、街道」；详细地址「小区楼栋、门牌号、村等」
- **AND** 地区滚轮标题「选择省市区街道」
- **AND** 保存成功 Toast「地址已保存」
- **AND** **不**展示邮政编码输入项；保存时不提交 `code`

#### Scenario: 手动选择地区
- **WHEN** 用户点击所在地区行
- **THEN** 弹出省市区选择器，确认后回显到地区行
- **AND** 不展示「定位」按钮，不调用定位 / 逆地理编码
...
#### Scenario: 详细地址特殊字符
- **WHEN** 详细地址含表情或其他特殊字符并保存
- **THEN** Toast「详细地址不能包含特殊字符」，不调用保存接口
- **WHEN** 详细地址仅为中英文、数字及常见地址标点
- **THEN** 允许保存
...
#### Scenario: 列表与订单中的手机号不脱敏
- **WHEN** 地址列表、确认订单、订单详情展示收货手机号
- **THEN** 原样展示接口 `mobile` / `phone`，客户端 **不得** 再做成 `138****5678`
- **AND** 若后端字段本身已含 `*`，按返回值展示
...
#### Scenario: 空列表
- **WHEN** 用户尚无任何收货地址
- **THEN** PL 层展示默认空态插图 `noData_default`、标题「暂无收货地址」、副文案「添加后可用于商城下单、服务资料邮寄等场景。」
- **AND** 底部主按钮为「添加收货地址」（不是「新增地址」）

#### Scenario: 列表提示条
- **WHEN** 地址列表有数据
- **THEN** 顶部橙色提示条文案为「默认地址可用于商城下单、服务资料邮寄等场景。」（商城，不是商场）

#### Scenario: 删除确认
- **WHEN** 用户点击删除
- **THEN** 弹窗标题「确认删除」，正文「删除后不可恢复，确认删除该地址？」
- **AND** 删除成功 Toast「地址已删除」
