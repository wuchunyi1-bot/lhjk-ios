...
提供统一的路由导航基础设施，基于 CTMediator 中间件模式实现模块间解耦，支持 URL-based 导航、模块 Target-Action 注册和参数传递。
...
┌────────────────────────────────────────────────────┐
│                    PL Layer                         │
│  PageA ──→ Router.push("/pageB", params) ──┐       │
│  PageC ←── Router.present("/pageD")         │       │
├─────────────────────────────────────────────┤       │
│                    Router (DAL)                     │
│  ┌──────────────────────────────────────────┐       │
│  │         CTMediator Core                  │       │
│  │  • registerRoute(path, target, action)   │       │
│  │  • performAction(target, action, params) │       │
│  │  • openURL(url, completion)              │       │
│  └──────────────────────────────────────────┘       │
├─────────────────────────────────────────────┤       │
│               BLL Layer                             │
│  Target_Home / Target_Health / Target_Message ...   │
│  每个模块暴露供路由调用的 Action 方法                  │
├─────────────────────────────────────────────┤       │
│               DAL Layer                             │
│  Router 本身位于 DAL，不依赖任何业务模块               │
└────────────────────────────────────────────────────┘
```
