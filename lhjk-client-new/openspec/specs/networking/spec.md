...
提供统一的网络请求能力，基于 Alamofire 封装，支持 RESTful API 调用、Token 自动管理、请求/响应拦截、错误处理和网络状态监控。

核心设计：利用 Alamofire 内置的 `AuthenticationInterceptor` + 自定义 `Authenticator` 协议处理 Token 的自动注入、过期刷新和并发控制，避免手动管理锁和队列。
