...
| 约束 | 说明 |
|------|------|
| 对齐原型正式态 | funde `MyVouchersView` 生产 Tab 即上述 5 个；「待使用」数量以红底数字角标展示，不拼进标题 |
...
| UI | API | 说明 |
|----|-----|------|
| 全部 | `GET /v1/benefitsTake/getCustomerPage` | 无 status；列表顺序与接口 `records` 一致，不按待领取/待使用等状态重排；2→待领取卡（有效期 + `expiringSoon`/`remainDays`），3/4/5→卡；即将到期文案以接口字段为准，不按本地日期推算 |
...
| UI | API | 说明 |
|----|-----|------|
| 全部 | `GET /v1/couponTake/getCouponTakeList` | 不传 status；列表顺序与接口 `records` 一致，不按待使用/已使用/已过期本地重排 |
| 待使用/已使用/已过期 | 同上 `?status=1/2/3` | 同样保持接口顺序 |
...
票券左侧金额区设计稿宽 **98pt**。面值、门槛必须完整展示，**禁止**省略号。字号 **只按展示字符串去空格后的字符数** 判断，**不按面额数值大小**（例如 `¥9999` 长度 5 用 22，`¥12800` 长度 6 用 18）。

**上方（面值展示串，如 `¥50` / `¥12800` / `¥99999.99` / `8.8折`）**

| 去空格后长度 | 字号 | 设计示例 |
|-------------|------|----------|
| ≤ 5 | 22 | `¥50`（3）、`8.8折`（4）、`¥1280`（5） |
| 6…7 | 18 | `¥12800`（6） |
| ≥ 8 | 15 | `¥99999.99`（9） |
...
| 去空格后长度 | 字号 | 设计示例 |
|-------------|------|----------|
| ≤ 10 | 12 | `满¥100可用`（7）、`满¥20000可用`（9） |
| ≥ 11 | 10 | `满¥99999.99可用`（12） |

两行各自按自己的字符串长度取字号，互不借用面额。满减/减价为 `¥` + 数字（数字 Bold，货币符 Medium）；折扣为数字 + `折`（`discountRatio` 已是折数，不得乘 10）。门槛文案 `满¥{n}可用`（无空格）；`conditionPrice` 为 0 时展示「满¥0可用」，不得写成「无门槛」。已使用 / 已过期只改颜色。不得用 `adjustsFontSizeToFitWidth` 连续缩放。
...
- **AND** 默认选中「全部」
- **AND** 「待使用」在数量 > 0 时于标题右上角展示数字角标（`fdDanger` 红底白字胶囊，与购物车 / TabBar 同源；>99 为 `99+`），不得把数字拼进 Tab 标题；数量为 0 时隐藏角标
...
### Requirement: 优惠券待使用角标

优惠券「待使用」Tab SHALL 与权益卡使用同一套数字角标（`VoucherFilterTabCell`），不得把数量拼进标题。

#### Scenario: 与权益卡样式一致

- **WHEN** 用户进入我的卡券并停留在优惠券模块，且待使用数量 > 0
- **THEN** 「待使用」标题右上角展示与权益卡相同的 `fdDanger` 红底白字胶囊角标（>99 为 `99+`）
- **AND** 标题文案仍为「待使用」，不含数字
...
优惠券各 Tab 列表 SHALL 按 `GET /v1/couponTake/getCouponTakeList` 返回的 `records` 顺序展示。SHALL NOT 按待使用 / 已使用 / 已过期或领取时间等在客户端重排。

#### Scenario: 全部 Tab 保持接口顺序

- **WHEN** 用户打开或刷新优惠券「全部」
- **THEN** 列表顺序与接口 `records` 一致

---

### Requirement: 优惠券左侧数值三档字号

优惠券票券左侧金额区 SHALL **按展示字符串去空格后的字符数** 选择字号，SHALL NOT 按面额数值大小选择。面值与门槛各自按本行字符串长度取号。SHALL NOT 以省略号截断。

#### Scenario: 短字符串用 22 / 12

- **WHEN** 面值展示串长度 ≤ 5（如 `¥50`、`¥1280`、`8.8折`），且门槛展示串长度 ≤ 10
- **THEN** 面值字号 22、门槛字号 12

#### Scenario: 中等长度面值串用 18

- **WHEN** 面值展示串长度为 6 或 7（如 `¥12800`）
- **THEN** 面值字号 18；门槛仍按其自身长度取 12 或 10

#### Scenario: 长面值串用 15

- **WHEN** 面值展示串长度 ≥ 8（如 `¥99999.99`）
- **THEN** 面值字号 15

#### Scenario: 长门槛串用 10

- **WHEN** 门槛展示串长度 ≥ 11（如 `满¥99999.99可用`）
- **THEN** 门槛字号 10，与面值字号互不影响

#### Scenario: 同面额不同长度

- **WHEN** 面值为 `¥9999`（长度 5）与 `¥10000`（长度 6）
- **THEN** 前者字号 22、后者字号 18（由字符串长度决定，不是由数值大小决定）
