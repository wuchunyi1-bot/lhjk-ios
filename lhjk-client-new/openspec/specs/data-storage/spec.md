...
提供统一的数据存储基础设施，封装 FMDB（SQLite）和 UserDefaults，为各业务模块提供结构化和轻量级数据存取能力。

## Data Classification

| 数据类型 | 存储方式 | 示例 |
|---------|---------|------|
| 结构化业务数据（需查询/分页/关联） | FMDB (SQLite) | 用户信息、消息记录、会话列表、健康数据、设备信息、订单记录 |
| 简单配置/状态（键值对） | UserDefaults | 用户偏好设置、功能开关、环境配置、Token、上次登录时间 |
| 缓存数据（可重建） | FMDB 缓存表 | API 响应缓存、图片缓存元数据 |
| 敏感数据（Token/密码等） | Keychain | Token、RefreshToken、支付密码 |
