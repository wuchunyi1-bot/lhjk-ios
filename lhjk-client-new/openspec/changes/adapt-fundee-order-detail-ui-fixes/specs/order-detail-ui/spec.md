...
| 区域 | 按钮 | 位置 |
|------|------|------|
| 滚动内容底部 | 取消订单、退款/售后 | `contentStack` 末行（订单信息卡之后），**随内容滚动**，滚到底可见 |
| 屏幕固定底栏 | 去支付、确认收货、续费订单、结算订单、确认发货、**去退货** | `OrderDetailActionBar` 贴屏底 |

- 仅存在滚动区按钮时，不展示固定底栏
- 仅「去退货」时，固定底栏展示该按钮，滚动区不重复展示
...
合资格时列表 **SHALL** 展示「去退货」。其余按钮（去支付、确认收货、续费订单、结算订单等）按 `OrderListCardAction` 矩阵正常展示。

用户如需取消/售后，SHALL 进入订单详情后操作。
