...
定义注册/登录完整流程的 UI 布局、交互行为、异常处理和适配规则。参考 funde-client（富德联好健康）PRD `用户注册与登录_v1.0.md` 及 `LoginView.vue` + `design-system.md` 的设计规范，通过 UIKit + SnapKit 将其适配到 iOS 项目。
...
#### Scenario: 用户不同意
- **WHEN** 用户点击「不同意」
- **THEN** 展示不可继续使用状态页，文案为「未同意隐私政策，暂无法使用富德联好健康」，提供「重新查看并同意」和「退出 App」两个操作
...
#### Scenario: App Name
- **WHEN** 页面渲染品牌区
- **THEN** Logo Mark 下方 16pt 处展示应用名称「富德联好健康」（22pt、bold、`UIColor.fdText`）
...
登录/注册成功后 SHALL 按 PRD §5.7 判断是否展示 App 内「开启消息通知弹窗」；不以冷启动 `requestAuthorization` 替代引导。

#### Scenario: 展示条件
- **WHEN** 登录/注册成功且系统通知未开启
- **AND** 本设备从未关闭过该弹窗，或距上次关闭已满 30 天
- **THEN** 展示弹窗：标题「开启消息通知」；说明「建议开启通知权限，第一时间接收健康服务提醒、服务进度与专属权益动态，同时保障您的账号安全。」

#### Scenario: 不展示
- **WHEN** 系统通知已开启，或 30 天冷却期内，或仅打开 App 未重新登录
- **THEN** 跳过弹窗，直接进入完善个人信息页或首页

#### Scenario: 去开启
- **WHEN** 用户点击「去开启」
- **THEN** 跳转系统 App 设置；用户返回 App 后重新读取系统授权、记录关闭状态、关闭弹窗并继续后续流程

#### Scenario: 暂不开启
- **WHEN** 用户点击「暂不开启」
- **THEN** 读取系统授权、记录关闭状态、关闭弹窗并继续后续流程（不阻断）

#### Scenario: 设置页系统状态
- **WHEN** 进入「消息通知设置」或 App 回前台
- **THEN** 「手机系统通知」状态行展示系统真实授权（已开启/未开启）；点击行跳转系统设置
