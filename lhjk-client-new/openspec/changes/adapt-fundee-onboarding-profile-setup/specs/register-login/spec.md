...
完善个人信息页（`OnboardingViewController`）SHALL 对齐 Figma `5330:13324`（未填写）与 `5346:16053`（已填写）。

#### Scenario: 页面结构

- **WHEN** 展示完善个人信息页
- **THEN** 页背景为暖米色；右上角展示装饰徽章；标题「完善个人信息」与副标题左对齐于白底表单上方
- **AND** 副标题为「填写基础信息，绑定专属服务机构与业务经理」
- **AND** 下方为顶部圆角 16 的白底表单区
- **AND** 字段顺序为：姓名、性别、出生日期、所属机构、业务经理（选填）
- **AND** 必填项标签左侧红色星标切图；业务经理标签带灰色「(选填)」，无星标
- **AND** 输入框为浅灰底 `#F7F7F7`、圆角 12、高 47；未填占位灰色，已填主文字色
- **AND** 出生日期未填占位为「年-月-日」，右侧日历切图
- **AND** 机构 / 业务经理行右侧为列表箭头切图，无左侧 SF Symbol 前缀
- **AND** 隐私提示气泡位于「提交信息」按钮上方；按钮为胶囊橙底

#### Scenario: 性别控件

- **WHEN** 尚未选择性别
- **THEN** 「男」「女」均为浅灰底与灰色文字（无 ♂/♀ 符号）
- **WHEN** 用户选择性别
- **THEN** 选中项为暖米色底、主色描边与主色文字；未选项保持浅灰
