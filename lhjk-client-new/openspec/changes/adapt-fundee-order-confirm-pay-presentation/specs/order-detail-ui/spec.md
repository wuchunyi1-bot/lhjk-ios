...
- 固定行：套餐金额、运费、优惠券抵扣（`couponAmount`）、权益卡抵扣（`benefitsAmount`；无抵扣也展示 `-¥0.00`）
...
- 右倾斜圆形印章：快递「待发货 / 已发货」；自提「待自提 / 已自提」（切图见 `order_detail_stamp_*`，已自提为 `order_detail_stamp_pickuped`）；**只**随该行 `shipmentStatus` 与履约方式变化，不随整单 status
- 已发货行可复制物流单号
