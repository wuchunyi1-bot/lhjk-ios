...
- **WHEN** 没有可展示的收货地址（无订单快照且无默认地址可绑定）
- **THEN** 展示 343×72pt 地址卡，文案为「请选择收货地址」，右侧显示 70×28pt、0.5pt `#FF7950` 描边的「去选择」按钮

#### Scenario: 待支付详情取消订单

- **WHEN** `entry=order_pay` 用户点击「取消订单」
- **THEN** 走与列表相同的待支付取消流程（二次确认 → status=8）
- **AND** 成功后返回订单列表「全部」Tab 并刷新（已取消仅全部可见；不得留在待支付 Tab）

#### Scenario: 优惠券行文案

- **WHEN** 订单或结算已使用优惠券（抵扣金额 > 0，或已绑定 `couponTakeId` / `couponTakeList`）
- **THEN** 展示 `已使用一张，共优惠¥{amount}`，金额取订单/结算抵扣
- **AND** 之后返回的 `getCouponTakeList` 只更新可用张数，**不得**把已使用文案改成 `有{n}张可用`
- **WHEN** 订单未使用优惠券，且有可用券
- **THEN** 展示 `有{n}张可用`
- **WHEN** 订单未使用优惠券，且无可用券
- **THEN** 展示 `暂无可用`
