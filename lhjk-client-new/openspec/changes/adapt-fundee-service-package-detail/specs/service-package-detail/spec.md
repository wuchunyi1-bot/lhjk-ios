...
- **AND** 每行展示：选择控件（仅父项）+ 商品名称｜数量单位｜单价；子项无控件、仅缩进文案

#### Scenario: checkType 选择控件

- **WHEN** 分组 `checkType = 2`（强制 / 必选）
- **THEN** 分组头部展示「必选」角标；控件为圆角方框 checkbox，**仅父项**展示控件且全部默认选中（红底白勾）
- **AND** 行不可点击、不可取消选中
- **WHEN** 分组 `checkType = 1`（单选）
- **THEN** 分组头部展示「单选」角标；控件为圆形 radio；组内父项互斥，必须且只能选 1 个父项
- **AND** 默认选中：**第一个父项**（列表中首个非子项），**不得**再按 `defaultCheck` / 最低价选默认
- **AND** 选中态为红圈 + 红实心圆点
- **WHEN** 分组 `checkType = 3`（可选 / 多选）
- **THEN** 分组头部展示「可选」角标；控件为圆角方框 checkbox，**仅父项**可勾选 / 取消
- **AND** 初始选中态：父项按 `defaultCheck == 1` 选中，否则未选；用户可自行取消已默认勾选的父项
- **AND** 角标文案 MUST 为「必选 / 单选 / 可选」，不得展示「强制 / 多选」等内部枚举名

#### Scenario: 父子子项展示与计价

- **WHEN** 明细行存在 `children` 子节点（映射为 `isChild = true`）
- **THEN** 子行 **不得**展示选择控件（无 radio / checkbox）
- **AND** 子行仅缩进展示名称｜数量｜单价，不可单独勾选或取消
- **WHEN** 父项被选中（必选默认 / 单选选中 / 可选勾选）
- **THEN** 该父项下全部子项视为一并选中
- **AND** 应付金额 = 已选父项单价 + 其下全部子项单价之和（父+子均计入）
- **WHEN** 组装加购 / 下单 / 确认订单草稿明细
- **THEN** 须同时包含已选父项及其子项（有有效 `detailId` 的均提交）
...
- **AND** UI 子行相对父行增加左侧缩进（对齐 funde `combo-row--child`），文案整体右移
- **AND** 子行 **不得**展示选择控件；选中态与计价跟随父项（见「父子子项展示与计价」）
