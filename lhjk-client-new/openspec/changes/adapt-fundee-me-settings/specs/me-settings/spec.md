...
- **THEN** 依次展示分组：
  ...
  - 通用与支持 → 关于富德联好健康（说明：品牌、版本与客服信息）→ `/me/settings/about`
- **AND** 每项右侧有箭头；行含左侧图标软底

#### Scenario: 隐私设置系统权限去开启

- **WHEN** 用户在隐私设置页点击「位置信息 / 相机权限 / 相册权限」，且系统授权状态为尚未询问（`notDetermined`）
- **THEN** 弹出对应系统授权弹窗，不跳转系统设置
- **WHEN** 用户已拒绝或权限受限
- **THEN** 再跳转系统设置页，供用户手动打开
- **AND** 授权结果变化后刷新行状态（已开启 / 未开启）
