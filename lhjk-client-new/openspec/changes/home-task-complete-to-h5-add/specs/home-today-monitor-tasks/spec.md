...
- **AND** 优先按字典 `monitorType.name` 跳转：营养补充剂 → `/supplement/add?taskId=`，运动 → `/exercise-food/check-in?taskId=`，用药 → `/medication`；血压/血糖/体重等走对应 H5 add
- **AND** 字典未同步时再按 value `2…7` 编号兜底；睡眠不跳转
